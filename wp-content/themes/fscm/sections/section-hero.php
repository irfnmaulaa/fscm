  ...
  <div class="hero-slides flex transition-transform duration-700 ease-in-out" data-current="0">
    ...
    ?>
      <div class="hero-slide w-full flex-shrink-0 h-[560px] lg:h-[560px] flex items-center justify-center"
           style="background: url('<?= is_numeric($slide['bg']) ? wp_get_attachment_url($slide['bg']) : $slide['bg'] ?>'), linear-gradient(180deg, rgba(30, 30, 30, 0.81) 54.33%, rgba(117, 117, 117, 0.081) 100%); background-size: cover; background-position: center; background-repeat: no-repeat; background-blend-mode: multiply;">
        <div class="container !max-w-[1500px] flex items-center justify-center text-center flex-col gap-5 px-4">
          <h2 class="text-white text-[40px] lg:text-[64px] font-bold leading-tight lg:leading-none lg:whitespace-pre-line"><?= $slide['title'] ?></h2>
          <p class="text-white text-[24px] mx-auto font-light tracking-[0.8px] lg:text-[40px] leading-[1.4] lg:leading-[1.3] whitespace-pre-line lg:whitespace-pre-line"><?= $slide['description'] ?></p>
          <a href="<?= $slide['cta']['url'] ?? '#' ?>" target="<?= $slide['cta']['target'] ?? '_self' ?>" class="btn btn-white mt-5 lg:mt-7 btn-primary text-[18px] lg:text-[24px] px-4 py-2 lg:px-5 lg:py-2 inline-flex gap-2 justify-center items-center bg-white border-[#767676]">
